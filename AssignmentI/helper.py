...
def estimate_alpha_beta_gamma_seasonal_hw(
    y, S, multiplicative=False, criterion="SSE", grid_n=31, eps=1e-2
):
    """
    Expanding-window Seasonal Holt–Winters with re-estimated (alpha,beta,gamma) at each step.

    Convention:
    - At time t, estimate (alpha,beta,gamma) using y[:t+1] (data through index t).
    - Then forecast y[t+1], and store results at index t+1.

    Returns
    -------
    alpha_t, beta_t, gamma_t : np.ndarray
        Parameter chosen for each time index (NaN where undefined).
    yhat_t : np.ndarray
        One-step-ahead forecast series (NaN where undefined).
    u_t : np.ndarray
        One-step-ahead errors y - yhat (NaN where undefined).
    ...
    for t in range(2 * S - 1, T - 1):
        # Fit parameters using data through t
        y_sub = y[:t+1]
        ...
        # Forecast y[t+1] using best params estimated from y[:t+1]
        y_sub_t = y[:t+2]
        if multiplicative:
            F_best, _, _, _ = holt_winters_multiplicative_forecast(
                y_sub_t, S, best_a, best_b, best_g
            )
        else:
            F_best, _, _, _ = holt_winters_additive_forecast(
                y_sub_t, S, best_a, best_b, best_g
            )

        # Optional: alignment debug print
        # print(
        #     f"t={t:3d} | len(y_sub)={len(y_sub):3d} | len(y_sub_t)={len(y_sub_t):3d} | "
        #     f"len(F_best)={len(F_best):3d} | store_idx={t+1:3d}"
        # )

        # Store at index t+1 (forecast for y[t+1])
        yhat_t[t+1] = F_best[-1]
        u_t[t+1] = y[t+1] - yhat_t[t+1]
